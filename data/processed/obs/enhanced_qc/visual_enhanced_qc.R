...
list(
  ...
  BR082704 = c("tmin['/1974'] <- NA",
               "tmin['1982/1988'] <- NA",
               "tmin['2017/2018'] <- NA"),
  BR082915 = c("tmin['1988/1990'] <- NA"),
  EC000031 = c("tmax[tmax > 25] <- NA"),
  PE100015 = c("tmin['1994/1999'] <- NA",
               "tmin[tmin > 15] <- NA",
               "tmin['2016/2018'] <- NA"),
  ...
  PE105001 = c("tmin['1952'] <- NA",
               "tmax['1952'] <- NA"),
  PE105055 = c("tmin['/1993'] <- NA",
               "tmax['/1997'] <- NA"),
  PE105064 = c("tmax['1993/2009'] <- NA"),
  PE105065 = c("tmin['/1996'] <- NA",
               "tmin['2000/2002'] <- NA",
               "tmax['/1996'] <- NA"),
  ...
  PE111046 = c("tmin['/1997'] <- NA",
               "tmax['/1998'] <- NA"),
  PE112012 = c("tmin['/2006'] <- NA", ###
               "tmax['/2006'] <- NA"),
  PE112055 = c("tmin['/1994'] <- NA",
               "tmin['2001/2005'] <- NA",
               "tmax['/1994'] <- NA",
               "tmax[tmax > 25] <- NA"),
  PE112059 = c("tmin['2006/2015'] <- NA",
               "tmax['1977/2004'] <- NA"),
  PE113019 = c("tmin[tmin > 20] <- NA",
               "tmin['/1992'] <- NA",
               "tmax['/1992'] <- NA"),
  PE113022 = c("tmin['/2000'] <- NA",
               "tmax['/2000'] <- NA"),
  PE113034 = c("tmin['1980/1985'] <- NA", # NEW CHANGE tmin['1980/1983'] <- NA
               "tmax['1980/1984'] <- NA"), # NEW CHANGE tmax['1980/1983'] <- NA
  PE113037 = c("tmin['/1970'] <- NA", "tmin['1976/2008'] <- NA", # NEW CHANGE
               "tmax['/1970'] <- NA", "tmax['1979/2008'] <- NA"), # NEW CHANGE
  PE113059 = c("tmin['/2006'] <- NA",
               "tmax['/2009'] <- NA"),
  PE113101 = c("tmin['/2009'] <- NA",
               "tmax['2006/2010'] <- NA"),
  ...
  PE114043 = c("tmin['2009/2014'] <- NA",
               "tmax['/1988'] <- NA",
               "tmax['2009/2014'] <- NA"),
  PE114046 = c("tmin['/2003'] <- NA",
               "tmax['/2003'] <- NA",
               "tmax[tmax > 27.5] <- NA"),
  PE114047 = c("tmin['/2008'] <- NA",
               "tmax['/2008'] <- NA"),
  PE115022 = c("tmin['/2011'] <- NA",
               "tmax['/2011'] <- NA"),
  PE115023 = c("tmin['2001/2004'] <- NA",
               "tmin['2010/'] <- NA",
               "tmax['2001/2004'] <- NA",
               "tmax['2010/'] <- NA"),
  ...
  EC000162 = c("tmin['/2000'] <- NA",
               "tmin['2000/2001'] <- NA",
               "tmax['/2001'] <- NA"),
  EC000180 = c("tmin['/1989'] <- NA",
               "tmin['2000/'] <- NA",
               "tmax['/1989'] <- NA",
               "tmax['2000/'] <- NA"),
  ...
  PE103031 = c("tmin['/1965'] <- NA", ###
               ...
               "tmax['/1965'] <- NA",
               "tmax['2005/2007'] <- NA"),
  PE103040 = c("tmin['/2010'] <- NA", 
               "tmin['2016/2017'] <- NA", # NEW CHANGE
               "tmax['/2011'] <- NA", 
               "tmax['2016/2017'] <- NA"), # NEW CHANGE tmax['1986/1988'] <- NA
  #            "tmax['2015/2016'] <- NA")
  PE103041 = c("tmin['/2007'] <- NA",
               "tmin['2014/2016'] <- NA",
               "tmax['/2001'] <- NA",
               "tmax['2014/2016'] <- NA"),
  ...
  PE105042 = c("tmin['/2007'] <- NA",
               "tmax['/2007'] <- NA"),
  PE105054 = c("tmin['1969/1985'] <- NA",
               "tmin['2005'] <- NA",
               "tmax['/1985'] <- NA",
               "tmax['1998/2001'] <- NA"),
  PE105056 = c("tmin['/1977'] <- NA",
               "tmin[tmin > 19] <- NA"), # NEW CHANGE
  PE105075 = c("tmin['/2004'] <- NA",
               "tmin['2017/'] <- NA",
               "tmin[tmin < 12.5] <- NA",
               "tmax['/2004'] <- NA"),
  PE106106 = c("tmin['/1990'] <- NA",
               "tmax['/1990'] <- NA"),
  PE106052 = c("tmin['/1992'] <- NA",
               "tmin['2008/2014'] <- NA",
               "tmax['/1999'] <- NA"),
  PE106014 = c("tmin['/2011'] <- NA",
               "tmax['/2011'] <- NA",
               "tmax['1980/1995'] <- NA",
               "tmax[tmax > 37.5] <- NA"),
  ...
  PE109022 = c("tmin['/2009'] <- NA",
               "tmin['/2017'] <- NA",
               "tmax['/2001'] <- NA"),
  PE110025 = c("tmin[tmin > 18] <- NA", ###
               "tmin['/1976'] <- NA", 
               "tmin['1995/1996'] <- NA", # NEW CHANGE
               "tmin['2001/2003'] <- NA",
               "tmin[tmin < -5] <- NA",
               "tmax['/1976'] <- NA", 
               "tmax['1986/1996'] <- NA", # NEW CHANGE
               "tmax['2001/2003'] <- NA"),
  PE111028 = c("tmin['1964'] <- NA",
               "tmin['1981/1986'] <- NA",
               "tmax['1982/1983'] <- NA"),
  PE114028 = c("tmin['2008/'] <- NA",
               "tmax['2008/'] <- NA",
               "tmax[tmax < 12.5] <- NA"),
  PE115044 = c("tmin['/2008'] <- NA",
               "tmax['/2012'] <- NA"),
  PE115052 = c("tmin['/2000'] <- NA",
               "tmin['2009/2010'] <- NA",
               "tmax['/2000'] <- NA"),
  PE115046 = c("tmin['/2003'] <- NA",
               "tmax['/2003'] <- NA"),
  ...
  PE116061 = c("tmin['/1963'] <- NA",
               ...
               "tmax['/1963'] <- NA",
               "tmax['1973/1980'] <- NA"),
  PE117019 = c("tmin['1969/1998'] <- NA",
               "tmin['2008/2018'] <- NA", # NEW CHANGE
               "tmax['2008/2018'] <- NA"),
  ...
  PE112067 = c("tmin['/1997'] <- NA", # NDD
               "tmin['2015/2020'] <- NA",
               "tmax['/1995'] <- NA"),
  PE112060 = c("tmin['/2007'] <- NA",
               "tmin['2010/2015'] <- NA",
               "tmax['1998/2020'] <- NA"), # NDD NMH
  PE111029 = c("tmin['/1997'] <- NA", # NDD NMH
               "tmin['2013/2020'] <- NA",
               "tmax['/1997'] <- NA",
               "tmax['2013/2020'] <- NA"),
  ...
  PE107008 = c("tmin['1980/1997'] <- NA",  # NEW CHANGE
               "tmax['1980/1997'] <- NA"),  # NEW CHANGE
  PE107036 = c("tmin['/2007-11-04'] <- tmin['/2007-11-04'] + 4",  # NEW CHANGE
               "tmin[tmin < 6] <- NA",
               "tmax['/2012'] <- NA"),  # NEW CHANGE
  PE100046 = c("tmax['/1994'] <- NA"),  # NEW CHANGE
  PE100139 = c("tmax['1989/1992'] <- NA"),  # NEW CHANGE
  PE115025 = c("tmin['/2000'] <- NA",
               "tmax['/2000'] <- NA"),  # NEW CHANGE
  PE115128 = c("tmin[tmin < 2.5] <- NA"),  # NEW CHANGE,
  PE113010 = c("tmin['/1998'] <- NA",
               "tmax['/1998'] <- NA",
               "tmin[tmin < 2.5] <- NA"), # NEW CHANGE
  PE115017 = c("tmin['/2014'] <- NA",
               "tmax['/2014'] <- NA"),
  PE114051 = c("tmin['/2001'] <- NA",
               "tmax['/2001'] <- NA"),
  PE115087 = c("tmin['/2010'] <- NA",
               "tmax['/2010'] <- NA"),
  PE116046 = c("tmin['/2010'] <- NA",
               "tmax['/2010'] <- NA"),
  PE117014 = c("tmin['2002/2003'] <- NA"),
  PE115027 = c("tmin['/1995'] <- NA",
               "tmax['/1995'] <- NA"),
  PE110020 = c("tmin['/1985'] <- NA",
               "tmin['1991/1992'] <- NA",
               "tmax['1991/1992'] <- NA",
               "tmax['/1985'] <- NA"),
  PE112058 = c("tmax['/1998'] <- NA"),
  PE100026 = c("tmin['/2005'] <- NA",
               "tmax['/2005'] <- NA"),
  PE104071 = c("tmax['2019/2020'] <- NA"),
  PE114018 = c("tmin['/1998'] <- NA"),
  PE109014 = c("tmin['1998/2014'] <- NA"),
  PE114020 = c("tmin['/1982'] <- NA", 
               "tmin['1990/1995'] <- NA",
               "tmax['/1982'] <- NA", 
               "tmax['1990/1995'] <- NA"),
  PE114039 = c("tmin['/1993'] <- NA",
               "tmax['/1993'] <- NA"),
  PE100081 = c("tmin['/1987'] <- NA",
               "tmax['/1987'] <- NA"),
  PE100136 = c("tmax['/1995'] <- NA"),
  PE106043 = c("tmin['/2008'] <- NA",
               "tmax['/2008'] <- NA"),
  PE106068 = c("tmax[tmax > 24] <- NA"),
  PE115138 = c("tmin['/2006'] <- NA")
)
