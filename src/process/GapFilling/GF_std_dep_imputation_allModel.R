...
std_dep_imputation_daily <- function(stat_data)
{
  ...
  if(length(model_ts_no_with_obs_v$mod) < 3){
    ...
    qm_fit <- qmap::fitQmapRQUANT(orig_ts_v, model_ts_no_with_obs_v_noNA, qstep = 0.01, nboot = 1, wet.day = FALSE)
    model_cc <- qmap::doQmapRQUANT(model_ts_no_with_obs_v_noNA, qm_fit, type = "tricub")
