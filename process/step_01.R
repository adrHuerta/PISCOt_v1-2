...
saveRDS(object = QC_data[["non_qc"]],
        file = file.path(non_qc_out, "non_qc_output", "non_QC00.RDS"))
...
saveRDS(object = lapply(QC_data, `[[`, "non_qc") %>% .[lapply(., nrow) != 0],
        file = file.path(non_qc_out, "non_qc_output", "non_QC01.RDS"))

saveRDS(object = list(values = lapply(QC_data, `[[`, "qc"), xyz = raw_xy_file),
        file = file.path(non_qc_out, "RAW(QC01)_data.RDS"))
...
saveRDS(object = lapply(QC_data, `[[`, "non_qc") %>% .[lapply(., nrow) != 0],
        file = file.path(non_qc_out, "non_qc_output", "non_QC02.RDS"))
...
saveRDS(object = lapply(QC_data, `[[`, "non_qc") %>% .[lapply(., nrow) != 0],
        file = file.path(non_qc_out, "non_qc_output", "non_QC03.RDS"))
...
saveRDS(object = lapply(QC_data, `[[`, "non_qc") %>% .[lapply(., nrow) != 0],
        file = file.path(non_qc_out, "non_qc_output", "non_QC04.RDS"))
...
saveRDS(object = lapply(QC_data, `[[`, "non_qc") %>% .[lapply(., nrow) != 0],
        file = file.path(non_qc_out, "non_qc_output", "non_QC05.RDS"))
...
#       ggplot2::ggsave(filename = file.path(non_qc_out, "enhanced_qc", "plots", paste(plot_title, ".jpg", sep = "")),
#                       plot = .,
#                       width = 20, height = 7,
#                       dpi = 150)
# 
...
saveRDS(object = QC_data[["non_qc"]] %>% .[sapply(., nrow) %>% unlist() != 0],
        file = file.path(non_qc_out, "non_qc_output", "non_QC06.RDS"))
...
#     ID_station <- x %>% as.character()
#     plot_title = paste(ID_station, "-", raw_xy_file$NAM[match(x, raw_xy_file$ID)], sep = "")
#     get_pRcs_temp(xts_obj = rest[["qc"]][[ ID_station ]]) %>%
#       enhanced_qc_plot(get_pRcs_temp_output = .,
#                        title_plt = plot_title) %>%
#       ggplot2::ggsave(filename = file.path(non_qc_out, "enhanced_qc", "plots_after_visual_qc", paste(plot_title, ".jpg", sep = "")),
#                       plot = .,
#                       width = 20, height = 7,
#                       dpi = 150)
# 
...
saveRDS(object = list(values = QC_data, xyz = raw_xy_file),
        file = file.path(non_qc_out, "RAW(QC06)_data.RDS"))
...
filter_stations_more_data <- get_stations_from_filter(data_list = QC_data,
                                                    length_of_years = round((2019-1981+1)*80/100, 0))

all(filter_stations_more_data %in% filter_stations)
...
saveRDS(object = list(values = list(tmax = lapply(QC_data, function(x) x[, "tmax"]) %>%
                                    ...
                                    tmin = lapply(QC_data, function(x) x[, "tmin"]) %>%
                                      ...
                                      .["1981/2019"]),
                      xyz = raw_xy_file),
        file = file.path(non_qc_out, "QC_data.RDS"))
